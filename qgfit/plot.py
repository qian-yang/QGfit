            ...
            if self.plot_line & self.config['plot_vertical_line']:
                ...
                if self.config['color_line_SNR'] > 0:
                    ...
                    ind = np.where(
                        ...
                        (lines_SNR['FWHM'] <= self.config['fwhm_narrow_max']))[0]
                    if (len(ind) > 0):
                    ...
                    ind = np.where(
                        # (lines_SNR['SNR'] > self.config['color_line_SNR']) &
                        (lines_SNR['FWHM'] > self.config['fwhm_broad_min']))[0]
                    if (len(ind) > 0):
                ...
                for c, comp in comps_part.iterrows():
                    if c < len(comps_part):
                        ...
                        if len(lines_comp) > 0:
                            ...
                            lines_broad = lines_comp[(fwhm > self.config['fwhm_broad_min']) & (norm > 0)]
                            lines_narrow = lines_comp[(fwhm <= self.config['fwhm_narrow_max']) & (norm > 0)]
