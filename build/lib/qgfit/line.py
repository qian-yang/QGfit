    ...
    def run_fit_line(self):
        ...
        for k, comp in self.comps.iterrows():
            ...
            if len(wave_comp) > 10:  # at least 10 pixels
                ...
                print('----------------- Fitting {0}-------------------'.format(comp_name))
                ...
                print('out.bic = {0}'.format(out.bic))
                # --- clean lines at boundaries ---
                n_line_vary = 0
                for k_tp, line in lines_new.iterrows():
                    print('----------------- clean {0}-------------------'.format(line['linename']))
                    suffix = '_' + str(k_tp)
                    ...
                    print('tell_broad = {0}'.format(tell_broad))
                    tell_mix = (type == 'mix') & (
                        # (norm <= 0) |
                        (center <= minwav) | (center >= maxwav) |
                    ...
                    print('tell_mix = {0}'.format(tell_mix))
                    ...
                    print('tell_narrow = {0}'.format(tell_narrow))
                    if not_vary | tell_broad | tell_mix | tell_narrow:
                        print('======> clean line: {0}'.format(line['linename']))
                ...
                out_clean = self._minimize(params_clean, data_line, comp_name, 
                                            method='leastsq', **kws)
                print('out_clean.bic = {0}'.format(out_clean.bic))
                out = out_clean
                print('n_line_vary = {0}'.format(n_line_vary))
                ...
                if n_line_vary > 1:
                    ...
                    print('bic_all = {0}'.format(bic_all))
                    params_tp = params_all.copy()
                    for k_tp, line in lines_new.iterrows():
                        print('---------- BIC {0}------------'.format(line['linename']))
                        suffix = '_' + str(k_tp)
                        ...
                        if params_modify['norm' + suffix].vary:
                            ...
                            if not done_test:
                                ...
                                print('bic_modify = {0}'.format(bic_all))
                    ...
                    if n_line_vary > 1:
                        ...
                        for k_tp, line in lines_new.iterrows():
                            ...
                            if (line['type'] != 'narrow') & (line['ngauss'] == 1): 
                                print('---------- broad {0}------------'.format(line['linename']))
                                suffix = '_' + str(k_tp)
                                params_modify = params_tp.copy()
                                if params_modify['norm' + suffix].vary:
                                    ...
                                    print('name_new = {0}'.format(suffix_tp))
                                    for key in keys:
                                    ...
                                    out_modify = self._minimize(params_modify, 
                                                                ...
                                                                **kws)
                                    bic_modify = out_modify.bic
                                    print('bic_add_modify = {0}'.format(bic_all))
                                    if bic_modify < bic_all:
                                        print('add line')
                                        params_tp = params_modify
                    ...
                    if key not in chain_keys:
                        if 'FWHM' not in key:
                            if not line_fit[key].vary: # --- fixed keys ----
                                ...
                                if expr is None:
                                    # print('key fixed: {0}'.format(key))
                                    flatchain[key] = np.zeros(n_chain) + line_fit[key].value
                                else:
                                    # print('key expr: {0} {1}'.format(key, expr))
